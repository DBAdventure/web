...
const Player = class Player {
    ...
    getDisplayName() {
        if (!isEmpty(this.guild_player) && this.guild_player.enabled) {
            return `[${this.getGuild().short_name}] - ${this.name}`;
        }
    ...
    getGuild() {
        if (isEmpty(this.guild_player) || isEmpty(this.guild_player.guild)) {
            return {};
        }

        return this.guild_player.guild;
    }

    getGuildRank() {
        if (isEmpty(this.guild_player) || isEmpty(this.guild_player.rank)) {
            return false;
        }

        return this.guild_player.rank.role;
    }
};
